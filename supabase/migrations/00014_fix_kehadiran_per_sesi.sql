-- Migration 00014: Fix kehadiran count to be per-session (not per-day)
...
BEGIN
  ...
  -- Hitung total sesi (per sesi, bukan per hari)
  SELECT count(*) INTO v_total_sesi
  FROM sessions s
  WHERE s.group_id = v_group_id
    AND s.tanggal = ANY(v_tanggal);

  -- Hitung sesi hadir (per sesi, unique session_id)
